...
string get_input_path(int day_of_challenge) {
    string file_name = fmt::format("{}/inputs/inputDia{}.txt", proyect_location, day_of_challenge);
    return file_name;
}

// make a function that returns a stream to the input file using the day and part of the challenge
fstream get_input_stream(int day_of_challenge) {
    string input_path = get_input_path(day_of_challenge); 
