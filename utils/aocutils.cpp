...
string get_input_path(int day_of_challenge, ChallengePart part_of_challenge) {
    string file_name = fmt::format("{}/inputs/inputDia{}{}", proyect_location, day_of_challenge,
        part_of_challenge == ChallengePart::A ? "A" : "B");
    return file_name;
}

// make a function that returns a stream to the input file using the day and part of the challenge
fstream get_input_stream(int day_of_challenge, ChallengePart part_of_challenge) {
